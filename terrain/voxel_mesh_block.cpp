...
void VoxelMeshBlock::update_navmesh(Ref<NavigationMesh> nav_mesh, const Transform3D &terrain_transform, RID navigation_map) {
	...
	if (!_nav_region.is_valid()) {
		...
		if (navigation_map.is_valid()) {
			nav->region_set_map(_nav_region, navigation_map);
		}
	}
